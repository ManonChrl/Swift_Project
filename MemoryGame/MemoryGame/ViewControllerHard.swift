...
//  MemoryGame
//
//  Created by Manon chancereul on 16/04/2015.
...
class ViewControllerHard: UIViewController {
    ...
    func updateTimer(dt:NSTimer){
        ...
        if counter==0{
            ...
            let actionFinish = UIAlertAction(title: "Back", style: .Default) { action -> Void in
                ...
                self.performSegueWithIdentifier("finishHard", sender: self)
            }
    ...
    override func viewDidLoad() {
        ...
        // Get the hard high score
        var statement:COpaquePointer = nil
        let query = "SELECT * FROM HIGHSCORE WHERE ID = 'hard'"
        if sqlite3_prepare_v2(database, query, -1, &statement, nil) == SQLITE_OK {
            while sqlite3_step(statement) == SQLITE_ROW {
                highScoreLabel.text = String(sqlite3_column_int(statement, 1)) // Put the hard high score in the label
            }
        ...
        if(numberOfCard==2){
            ...
            if(card1 != card2){
                ...
                if(card1.imageView?.image?.hashValue == card2.imageView?.image?.hashValue){
                    ...
                    timer = NSTimer.scheduledTimerWithTimeInterval(0.5, target: self, selector: Selector("hideCards"), userInfo: nil, repeats: false)
                }
                numberOfCard = 0
                scoreLabel.text = String(score)
            } else {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject!) {
        if (segue.identifier == "finishHard") {
